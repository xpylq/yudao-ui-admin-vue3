<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="地区" align="center" prop="area">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.MD_MOVIE_AREA" :value="scope.row.area" />
        </template>
      </el-table-column>
      <el-table-column label="作者" align="center" prop="author" />
      <el-table-column label="时长" align="center" prop="duration" />
      <el-table-column label="观看次数" align="center" prop="viewTimes" />
      <el-table-column
        label="创建时间"
        align="center"
        prop="createTime"
  ...
  <!-- 表单弹窗：添加/修改 -->
  <MovieForm ref="formRef" @success="getList" />
</template>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await MovieApi.getMoviePage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await MovieApi.exportMovie(queryParams)
    download.excel(data, '影片.xls')
  } catch {
  } finally {
